...
public class SegmentTree
{
    ...
    public long LowerBound(long target) { return LowerBound(root, 0, n, target); }
    ...
    private long LowerBound(Node node, long start, long end, long target)
    {
        if (node.max < target) return end + 1;
        if (start == end) return start;
        long mid = (start + end) >> 1;
        PushDown(node, mid - start + 1, end - mid);
        if (node.left.max >= target) return LowerBound(node.left, start, mid, target);
        else return LowerBound(node.right, mid + 1, end, target);
    }
